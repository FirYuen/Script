...
def callfunc():
    for n in [64,128,192,256,384,512,768,1024,1088,1472,2048,4096]:
        dataCall = json.dumps({"head": {"sessionid": "undefined", "seqid": 0}, "body": {"param1": {"basetype": "192.168.33.176"}, "param2": {"basetype": n}, "param3": {"basetype": 1}}})
...
def getInfoAndWRtoExcel(n):
    ...
    meetingState1 = json.loads(requests.get('http://192.168.60.23/mtapi/callinfo/callstatisticsinforeq?*').content)
    ...
    time.sleep(3)
    meetingState2 = json.loads(requests.get('http://192.168.60.23/mtapi/callinfo/callstatisticsinforeq?*').content)
    ...
    time.sleep(3)
    meetingState3 = json.loads(requests.get('http://192.168.60.23/mtapi/callinfo/callstatisticsinforeq?*').content)
